    ...
    <section id="products" class="products-section">
      <div class="products-container">
        ...
        <div class="products-main">
          ...
          <div
            v-if="isLoading"
            class="products-grid products-grid-skeleton"
...
const fetchVendors = async () => {
  try {
    ...
    if (r.data.success) {
      const vendorData = Array.isArray(r.data.data?.data)
        ? r.data.data.data
        : [];
...
.shop-page {
  ...
  background:
    radial-gradient(
      circle at 5% 28%,
      rgba(218, 203, 235, 0.32),
      transparent 22rem
    ),
    radial-gradient(
      circle at 95% 38%,
      rgba(193, 229, 208, 0.36),
      transparent 24rem
    ),
    #fbfaf7;
  color: #214a39;
...
.hero {
  ...
  box-shadow: 0 14px 40px rgba(32, 74, 57, 0.09);
}
.hero::after {
  content: "";
  position: absolute;
  ...
  right: -7rem;
  top: -9rem;
  background: rgba(177, 151, 203, 0.18);
}
.hero-content {
  position: relative;
  z-index: 1;
...
.btn-shop-now {
  border-radius: 999px;
  background: #19734f;
  box-shadow: 0 8px 16px rgba(25, 115, 79, 0.2);
}
.btn-primary:hover,
.btn-promo:hover,
.btn-shop-now:hover {
...
.badge {
  z-index: 1;
  border-radius: 999px;
  box-shadow: 0 7px 16px rgba(35, 68, 52, 0.12);
}
...
.vendor-search-bar {
  ...
  border-radius: 999px;
  box-shadow: 0 4px 13px rgba(35, 68, 52, 0.05);
}
.vendors-grid {
...
.vendor-card {
  ...
  box-shadow: 0 7px 18px rgba(32, 74, 57, 0.05);
}
.vendor-card:hover,
.vendor-card-active {
  border-color: #a6d5ba;
  box-shadow: 0 12px 25px rgba(32, 74, 57, 0.11);
}
.vendor-card-logo {
  width: 42px;
  height: 42px;
...
.filter-sidebar {
  ...
  box-shadow: 0 10px 26px rgba(32, 74, 57, 0.06);
}
.filter-header h2 {
  color: #244a3b;
  font-size: 18px;
...
.popular-products-panel {
  ...
  border-radius: 18px;
  background: linear-gradient(125deg, #f8f4fb, #fdfcf9);
  box-shadow: none;
}
.popular-products-head h3 {
  color: #2c4e40;
...
.product-card-skeleton {
  ...
  border-radius: 18px;
  background: #fffefd;
  box-shadow: 0 8px 24px rgba(42, 67, 57, 0.05);
}
.skeleton {
  position: relative;
  overflow: hidden;
...
.skeleton::after {
  ...
  animation: shop-shimmer 1.45s infinite;
}
