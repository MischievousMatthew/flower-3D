  ...
  <div class="scan-page">
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">
          Scan a delivery barcode to confirm the customer has received the order
        </p>
      </div>
      <span class="queue-badge purple"
        >{{ queue.length }} deliveries in transit</span
      >
    </div>
    ...
    <div class="scan-layout">
      <div class="scan-col">
        <div class="dark-card">
          ...
          <div class="dark-card-body">
            <ScannerInput
              ref="scannerRef"
              placeholder="Scan delivery barcode to confirm receipt…"
              :auto-focus="true"
              @scan="handleScan"
            />
          </div>
        ...
        <transition name="result-panel">
          <div v-if="lastResult" class="dark-card result-card">
            ...
            <div class="result-body">
              <div class="result-ref purple">{{ lastResult.delivery_id }}</div>
              ...
              <div class="result-row">
                ...
                <span class="rv"
                  >₱{{ fmt(lastResult.order?.total_amount) }}</span
                >
              </div>
              <div class="result-row">
                <span class="rl">New Status</span>
                <span class="rv" style="color: #8b5cf6">→ To Receive ✓</span>
              </div>
              <div class="note-banner">
                ℹ️ Customer must still click "Order Received" to fully complete
              </div>
            </div>
          </div>
        </transition>
        ...
        <div class="dark-card" v-if="scanLog.length">
          <div class="dark-card-header">
            <div class="dc-title">Delivery Log</div>
      ...
      <div class="info-col">
        <div class="light-card">
          <div class="lc-header">
            <div class="lc-title">Delivery Workflow</div>
          </div>
          <div class="instructions">
            <div class="step">
              <div class="step-num purple">1</div>
              <div>Arrive at the delivery address with the order</div>
            </div>
            <div class="step connector-step">
              <div class="ws-connector"></div>
            </div>
            <div class="step">
              <div class="step-num purple">2</div>
              <div>Scan the barcode on the package</div>
            </div>
            <div class="step connector-step">
              <div class="ws-connector"></div>
            </div>
            <div class="step">
              ...
              <div>
                Status → <strong>To Receive</strong> · Customer notified to
                confirm
              </div>
            </div>
          </div>
        ...
        <div class="light-card">
          <div class="lc-header">
            <div class="lc-title">In Transit</div>
            <span class="count-badge purple">{{ queue.length }}</span>
          </div>
          ...
          <div v-else class="queue-list">
            <div
              ...
              :key="d.id"
              class="queue-item"
              :class="{ done: receivedIds.has(d.id) }"
            >
              <div class="qi-left">
                <div
                  class="qi-icon purple"
                  :class="{ done: receivedIds.has(d.id) }"
                >
                  <svg
                    v-if="receivedIds.has(d.id)"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                    width="10"
                  >
                ...
                <div>
                  <div
                    class="qi-ref"
                    :class="{ strike: receivedIds.has(d.id) }"
                  >
              ...
              <div class="qi-right">
                ...
                <span class="done-tag purple" v-if="receivedIds.has(d.id)"
                  >Delivered ✓</span
                >
                <span class="transit-badge" v-else>In Transit</span>
              </div>
            </div>
          </div>
        </div>

        <div class="light-card" v-if="receivedIds.size > 0">
          <div class="session-stats">
            <div class="ss-item">
              <span class="ss-val purple">{{ receivedIds.size }}</span
              ><span class="ss-lbl">Delivered</span>
            </div>
            <div class="ss-div"></div>
            <div class="ss-item">
              <span class="ss-val">{{ queue.length - receivedIds.size }}</span
              ><span class="ss-lbl">Remaining</span>
            </div>
            <div class="ss-div"></div>
            <div class="ss-item">
              <span class="ss-val purple">{{ pct }}%</span
              ><span class="ss-lbl">Progress</span>
            </div>
          </div>
          <div class="prog-wrap">
            <div class="prog-bar">
              <div class="prog-fill purple" :style="{ width: pct + '%' }"></div>
            </div>
          </div>
        </div>
      </div>
...
const TODAY_KEY = `receivedIds_${new Date().toISOString().slice(0, 10)}`;
...
const lastTime = ref("");
const scanLog = ref([]);
const receivedIds = ref(loadReceivedIds());
let logId = 0;
let pollTimer = null;

const pct = computed(() => {
  if (!queue.value.length) return 0;
  return Math.round((receivedIds.value.size / queue.value.length) * 100);
});
...
async function handleScan(barcode) {
  ...
  try {
    ...
    const res = await deliveryService.scan(code, "to_receive");
    const delivery = res?.data?.delivery ?? res?.delivery ?? res;

    lastResult.value = delivery;
    lastTime.value = new Date().toLocaleTimeString();
    receivedIds.value = new Set([...receivedIds.value, delivery.id]);
    saveReceivedIds(receivedIds.value);

    const orderNum =
      delivery.order?.order_number ?? delivery.delivery_id ?? code;
    addLog(code, "ok", `${orderNum} → To Receive`);
    scannerRef.value?.showResult(
      "ok",
      "Delivery confirmed — awaiting customer",
    );
    toast.success("Delivery confirmed", { autoClose: 2500 });
