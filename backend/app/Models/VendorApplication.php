...
class VendorApplication extends Model
{
    ...
    protected $fillable = [
        ...
        'bank_name',
        'billing_address',
        'product_types', // UPDATED - now array of: bouquet, flower, bouquet_and_flower
        'price_min', // NEW - minimum price
        'price_max', // NEW - maximum price
        'same_day_delivery',
        'cutoff_times', // UPDATED - now JSON array of {day, time}
        'delivery_handled_by', // UPDATED - only 'self' allowed
        'max_orders_per_day',
        'lead_time',
        'cancellation_policy',
        'store_logo_path',
        ...
        'product_details_completed',
        'delivery_details_completed',
        'profile_fully_completed',
    ];
    ...
    protected $appends = [
        ...
        'mayor_permit_url',
        'store_logo_url',
        'profile_photo_url', // NEW - Uses store logo
        'portfolio_photos_urls',
        'formatted_business_type',
        'formatted_status',
        'formatted_date',
        'formatted_price_range', // NEW
        'decrypted_account_number',
        'profile_completion_percentage',
    ];

    protected $hidden = [
        'account_number', // Never expose the encrypted value directly
    ];
    ...
    /**
     * Set the account number (encrypts it)
     */
    ...
    public function updateProfileCompletion()
    {
        ...
        // Check product details completion (UPDATED)
        ...
        // Check delivery details completion (UPDATED - no delivery_fee check)
    ...
    {
        return $this->hasMany(VendorApplicationNote::class);
    }
    ...
    {
        return Attribute::make(
            get: fn () => $this->government_id_path ? Storage::url($this->government_id_path) : null
        );
    }
    ...
    {
        return Attribute::make(
            get: fn () => $this->selfie_with_id_path ? Storage::url($this->selfie_with_id_path) : null
        );
    }
    ...
    {
        return Attribute::make(
            get: fn () => $this->proof_of_address_path ? Storage::url($this->proof_of_address_path) : null
        );
    }
    ...
    {
        return Attribute::make(
            get: fn () => $this->barangay_clearance_path ? Storage::url($this->barangay_clearance_path) : null
        );
    }
    ...
    {
        return Attribute::make(
            get: fn () => $this->mayor_permit_path ? Storage::url($this->mayor_permit_path) : null
        );
    }
    ...
    {
        return Attribute::make(
            get: fn () => $this->store_logo_path ? Storage::url($this->store_logo_path) : null
        );
    }

    /**
     * NEW - Profile photo uses store logo
     */
    ...
    protected function portfolioPhotosUrls(): Attribute
    {
        return Attribute::make(
            get: fn () => $this->portfolio_photos_paths
                ? array_map(fn ($p) => Storage::url($p), $this->portfolio_photos_paths)
                : []
        );
    }
